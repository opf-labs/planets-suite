...
public class ModifyTemplate extends WorkflowTemplateHelper implements WorkflowTemplate{
	...
	/**
	 * Warning! Overrides the standard getData() Method of the WorkflowTemplateHelper
	 * with hardcoded reference to BL files stored on the local file system
	 * @override
	 * @return the BL files to be cropped
	 *
	public List<DigitalObject> getData() {
		// Reference to files on file system
		SimpleBLNewspaperDigitalObjectManagerImpl blnImpl = new SimpleBLNewspaperDigitalObjectManagerImpl("c:\\bl\\");
		...
		// Retrieve
		ArrayList<DigitalObject> digObjects = new ArrayList<DigitalObject>();
		for (URI id : identifiers) {
			try {
				digObjects.add(blnImpl.retrieve(id));
	...
	*/
	...
	private DigitalObject runRotateService(DigitalObject digO, String inputFormat, String metadata, WorkflowResult wfresult) throws Exception {
		log.info("STEP 2: Rotation...");
		URI inputFormatURI = new URI(inputFormat);
		
		// Get rotation parameter from metadata - uses RegEx for high performance and ease of use
		int skew = 0;
		if (metadata != null) {
	...
	private DigitalObject runCropService(DigitalObject digO, String inputFormat, String metadata, WorkflowResult wfresult) throws Exception {
		log.info("STEP 3: Crop...");
		URI inputFormatURI = new URI(inputFormat);
		
		// Get cropping parameters from metadata - uses RegEx for high performance and ease of use
		...
		if (success) {
			...
			log.warn("No cropping coordinates found in DigitalObject metadata - defaulting to parameters in XML config!");
	...
	private DigitalObject runMigrateService(DigitalObject digO, URI migrateFromURI, WorkflowResult wfresult) throws Exception {
    	...
    	Parameter pCompressionQuality = this.getServiceCallConfigs(this.migrate).getPropertyAsParameter("compressionQuality");
