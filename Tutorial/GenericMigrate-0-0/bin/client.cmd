@echo off
rem ==$Workfile: client.cmd $===========================================
rem  execute eu.planets-project.service.migrate.MigrateClient_0
rem  in lib/GenericMigrate-0-0.jar. 
rem  Version    : $Id$
...
rem JAVA_OPTS
rem --------------------------------------------------------------------
set options=-Xmx512m -Djava.endorsed.dirs="%execdir%\..\lib\endorsed" -Djava.util.logging.config.file="%execdir%\..\etc\logging.properties" %JAVA_OPTS%
...
:execute
rem --------------------------------------------------------------------
rem execute MigrateClient_0 in lib/GenericMigrate-0-0.jar
rem --------------------------------------------------------------------
"%java%" %options% -cp "%execdir%\..\lib\GenericMigrate-0-0.jar" eu.planets_project.services.migrate.MigrateClient_0 %*
goto exit
...
@echo "Calling syntax                                                      "
@echo "  client.cmd <text> [<host> [<service>]]                            "
@echo "executes                                                            "
@echo "  eu.planets-project.service.migrate.MigrateClient_0                "
@echo "in lib/GenericMigrate-0-0.jar                                       "
@echo "                                                                    "
@echo "Arguments:  (See user's manual for more detail).                    "
@echo "  <text>    text to be migrated (default: "Hello JAX-WS!")          "
@echo "  <host>    hostname/IP optionally followed by : and the port       "
@echo "            (default: localhost:8080)                               "
@echo "  <service> name of the service deployed at <host>                  " 
@echo "            implementing Migrate_0.                                 "
@echo "            (default: GenericMigrate-0-0)                           "
@echo "                                                                    "
@echo "Javahome:                                                           "
