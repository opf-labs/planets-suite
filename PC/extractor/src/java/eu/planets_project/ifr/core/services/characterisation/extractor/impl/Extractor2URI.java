...
@MTOM
public class Extractor2URI implements BasicCharacteriseOneBinaryXCELtoURI, Serializable {

	private static final long serialVersionUID = 3007130161689982082L;
	private final static PlanetsLogger plogger = PlanetsLogger.getLogger(Extractor2URI.class);
	...
	public URI basicCharacteriseOneBinaryXCELtoURI ( 
		...
		try {
			...
			plogger.error("The following SOAPException occured: " + e.getLocalizedMessage());
			e.printStackTrace();
		}
	...
	//				        // and add the "_[timestamp]" to the filename
							...
							plogger.info("Storing file with new name: " + renamedFileName + " in DataRegistry...");
							// store it in the DataRegistry, using the new filename
							dataRegistry.storeBinary(renamedFileURI, binary);
							plogger.info("Successfully stored binary in DataRegistry: " + renamedFileName);
							fileURI = renamedFileURI;
						}
						
						// There have been hits (e.g. files with the same name), but in a different folder,
						// so just store the file with its initial name in the DataRegistry
						else {
							plogger.info("Attempting to store binary in DataRegistry: " + fileName);
							// store the file...
							dataRegistry.storeBinary(fileURI, binary);
							plogger.info("Successfully stored binary in DataRegistry: " + fileName);
						}
					}
				}
