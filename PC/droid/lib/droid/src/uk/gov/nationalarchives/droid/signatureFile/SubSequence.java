...
public class SubSequence extends SimpleElement {
	...
	public int getNumAlternativeFragments(boolean leftFrag, int thePosition) {
		if (leftFrag) {
			return ((ArrayList) this.orderedLeftFragments.get(thePosition - 1)).size();
		} else {
			return ((ArrayList) this.orderedRightFragments.get(thePosition - 1)).size();
		}
	}
	...
	public LeftFragment getRawLeftFragment(int theIndex) {
		return (LeftFragment) leftFragments.get(theIndex);
	}

	public RightFragment getRawRightFragment(int theIndex) {
		return (RightFragment) rightFragments.get(theIndex);
	}
