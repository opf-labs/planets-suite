...
import eu.planets_project.services.utils.cli.CliMigrationPath;
...
public class GenericCLIMigrationWrapper {
    ...
    // FIXME! configFilePath really ought to be a File object or some kind of
    // input stream. CLIMigrationPaths should do the same, the behaviour of its
    // initialiseFromFile() method is pretty hard-wired to J2EE, and it is not
    // quite obvious where to put the configuration file.
    public GenericCLIMigrationWrapper(String configFilePath)
	    throws MigrationInitialisationException {

	try {
	    migrationPaths = CliMigrationPaths
		    .initialiseFromFile(configFilePath);
	} catch (Exception e) {
	    throw new MigrationInitialisationException(
		    "Failed initialising migration path data from the file: "
			    + configFilePath, e);
	}
    }
	...
	/* - Validate that the proper parameters are set for the migration path identified by sourceFormat and destinationFormat
	 * 
	 * 
	*/

	final CliMigrationPath migrationPath = migrationPaths.findMigrationPath(sourceFormat, destinationFormat);
	
	String command = migrationPath.getConmmandLine(toolParameters); //TODO: May throw exception if the parameters are incorrect or insufficient.

	try {
	    final ProcessRunner toolProcessRunner = new ProcessRunner();

	    File sourceTempFile =null, destinationTempFile =null; //FIXME! I didn't do it....
