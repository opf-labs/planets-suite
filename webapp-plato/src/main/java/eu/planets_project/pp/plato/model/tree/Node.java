...
public class Node extends TreeNode {
     ...
     */
    private boolean hasDuplicates(List<String> messages) {
        boolean duplicates = false;
        Collection<String> childNames = new HashSet<String>();
        for (TreeNode node : this.children) {
            if (childNames.contains(node.getName())) {
                messages.add("Node '" + this.getName()
                        + "' has several children with the name '" + node.getName() + "'");
                duplicates = true;
            } else {
     ...
     * @param errorMessages list of messages to which to append error messages
     */
    @Override
    public boolean isCompletelySpecified(List<String> errorMessages) {
        boolean noError = true;
        if (this.children.size() == 0) {
            errorMessages.add("Node " + this.getName() + " has no Children.");
            noError = false;
        }
        if (this.hasDuplicates(errorMessages)) {
            noError = false;
        }
        return noError;
    }
     ...
     * @return true (always!)
     */
    public boolean isCompletelyEvaluated(List<String> nodelist,
     ...
     */
    @Override
    public boolean isCompletelyTransformed(List<String> errormessages,
            List<TreeNode> nodes) {
        return true;
    }
     ...
     * @param errormessages list to which to append error messages
     */
    @Override
    public boolean isCorrectlyWeighted(List<String> errormessages) {
        ...
        errormessages.add("The sum of the weights of node " + this.getName()
                + "'s children is not 1 (but " + sum / 100.0 + ").");
        return false;
    }
