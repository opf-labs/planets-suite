...
public class StorageDigitalObjectDirectoryLister {
    ...
    private void fillDirectoriesMap(URI uri) {
		...
		for (int i = 0; i < childsList.size(); i++) {
			...
			if (childsList.get(i) != null && childsList.get(i).toString().contains(DOJCRConstants.DOJCR)) {
				log.info("+++ StorageDigitalObjectDirectoryLister fillDirectoriesMap() childsList.get(i): " + childsList.get(i) + ", i: " + i);
				// split the URI into directories
				String [] dirsArray = childsList.get(i).toString().split(DOJCRConstants.JCR_PATH_SEPARATOR);
				int idx = 1;
				// evaluate path until DigitalObject node
				while (!dirsArray[idx + 1].equals(DOJCRConstants.DOJCR)) {
					log.info("+++ StorageDigitalObjectDirectoryLister fillDirectoriesMap() while dirsArray[idx]: " + dirsArray[idx] + ", idx: " + idx);
					addChildToMap(getPrefix(dirsArray, idx) + dirsArray[idx], dirsArray[idx + 1], dirsArray); // to delete dirsarray
					idx++;
				}
				...
				if (dirsArray[idx + 1].equals(DOJCRConstants.DOJCR)) {
					log.info("+++ StorageDigitalObjectDirectoryLister fillDirectoriesMap() not wihle dirsArray[idx]: " + dirsArray[idx] + ", idx: " + idx);
					addChildToMap(getPrefix(dirsArray, idx) + dirsArray[idx], dirsArray[idx + 1] + DOJCRConstants.JCR_PATH_SEPARATOR + dirsArray[idx + 2], dirsArray);
				}
			}
		}  
...
//				puri.toString().contains(DOJCRConstants.REGISTRY_NAME) || puri.toString().contains(DOJCRManager.PERMANENT_URI))) {
