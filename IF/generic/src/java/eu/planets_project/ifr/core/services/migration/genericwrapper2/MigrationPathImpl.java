...
class MigrationPathImpl implements MigrationPath {
	...
	public List<String> getCommandLine(Collection<Parameter> toolParameters)
		...
		for (String cmd : commandLine) {
			...
			for (String tempFileLabel : tempFiles.keySet()) {
				//FIXME! This is wrong! The temp file name must be absolute paths, however, this is easily fixed if the command line is constructed by the generic wrapper class.
				cmd = cmd.replaceAll("#" + tempFileLabel, tempFiles.get(tempFileLabel));
			}
			if (useTempSourceFile()) {
	...
	/**
	 * Replace the current map defining the relationship between the labels in the command
