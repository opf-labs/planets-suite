    ...
    protected void setUp() throws Exception {
                ...
                || "server".equals(System.getenv("pserv.ws.test.context")) ) 
        {
            ...
            if ( System.getenv("pserv.ws.test.context").equals("standalone")) {
                ...
                // specified address:
                Endpoint testEndpoint = Endpoint.create(new PassThruMigrationService());
                url = new URL("http://localhost:18367/pserv-if-simple/DigitalObjectMigration?wsdl");
                testEndpoint.publish(url.toString());
                
                /*      
                        Endpoint testEndpoint = Endpoint.create( new DigitalObjectMigration() );
                        ...
                        HttpContext context = httpServer.createContext("/pserv-if-simple");
            ...
            // In the server case, pick the server config up:
                ...
                System.out.println("INIT: Configuring against server at localhost:8080");
                url = new URL("http://localhost:8080/pserv-if-simple/DigitalObjectMigration?wsdl");
                
            }

            System.out.println("INIT: Creating the proxied service class.");
            Service service = Service
                    .create(url, Migrate.QNAME);
    ...
    public void testMigrate() {
        try {
            ...
            DigitalObject input = new DigitalObject.Builder(new URL(
                    "http://some"), new Content(new URL("http://some")))
                    .build();
            ...
            assertTrue("Resulting digital object not .equal to the original.", input.equals(doOut) );
