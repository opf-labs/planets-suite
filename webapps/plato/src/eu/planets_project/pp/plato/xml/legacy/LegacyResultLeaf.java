/*******************************************************************************
 * Copyright (c) 2006-2010 Vienna University of Technology, 
 * Department of Software Technology and Interactive Systems
 *
 * All rights reserved. This program and the accompanying
 * materials are made available under the terms of the
 * Apache License, Version 2.0 which accompanies
 * this distribution, and is available at
 * http://www.apache.org/licenses/LICENSE-2.0 
 *******************************************************************************/

package eu.planets_project.pp.plato.xml.legacy;

import java.util.List;

/**
 * eu.planets_project.pp.plato.xml.legacy import class for reading evaluation results stored in DELOS DP Testbed XML files
 * @author Christoph Becker
 *
 */
public class LegacyResultLeaf implements ILegacyElement {
    private String name;
    private String value;
    private String weight;
    private ILegacyElement parent = null;

    public String getPathName() {
        return (parent != null ? (parent.getName()+name) : name ); 
    }
    
    public ILegacyElement getParent() {
        return parent;
    }
    public void setParent(ILegacyElement parent) {
        this.parent = parent;
    }
    public String getValue() {
        return value;
    }
    public void setValue(String value) {
        this.value = value;
    }
    public String getWeight() {
        return weight;
    }
    public void setWeight(String weight) {
        this.weight = weight;
    }
    public String getName() {
        return name;
    }
    public void setName(String name) {
        this.name = name;
    }
    public List<LegacyResultLeaf> getAllLeaves() {
        return null;
    }
}
